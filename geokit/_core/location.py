...
class Location(LocationNT):
    def __hash__(s): return hash(s) # I need this to make pandas indexing work when location objects are used as columns and indexes
    def __eq__(s,o):
        return isclose(s.lon,o.lon,1e-6) and isclose(s.lat,o.lat,1e-6)

    def __ne__(s,o):
        return not(s==o)

    def __str__(s):
        return "%8f,%8f"%(s.lat,s.lon)
...
def ensureLocation(locations, srs=None, forceAsArray=False):
    if isinstance(locations, Location): output = locations
        ...
        else: output = [ensureLocation(l) for l in locations]

    elif isinstance(locations, types.GeneratorType):
        output = ensureLocation(list(locations))
